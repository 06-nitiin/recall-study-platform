import { BarChart3, Flame, Save } from "lucide-react";
...
export function ProgressPanel() {
  ...
  useEffect(() => {
    Promise.all([getAnalytics(), getPreferences()])
  ...
  const maxHeat = Math.max(
    1,
    ...(analytics?.heatmap.map((day) => day.count) ?? [0]),
  );

  return (
    <section className="mx-auto mt-6 max-w-6xl rounded-[1.75rem] border border-slate-200 bg-white p-6">
