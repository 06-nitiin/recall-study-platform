...
export async function createModuleForUser(
  userId: number,
  input: {
    title: string;
...
export async function updateModuleForUser(
  ...
  return result.changes > 0
    ? getModuleForUser(moduleId, userId)
    : undefined;
}
...
export async function createNoteForModule(
  moduleId: number,
  input: {
    title: string;
    body: string;
...
export async function createTaskForModule(
  moduleId: number,
  input: {
    title: string;
    dueDate?: string | null;
...
export async function createMaterialForModule(input: {
  moduleId: number;
...
export async function setMaterialExtraction(
  materialId: number,
  status: "ready" | "failed",
...
export async function replaceGeneratedStudyContent(
  moduleId: number,
  content: {
    summary: string;
    concepts: unknown[];
  ...
  db.transaction((tx) => {
    ...
    tx
      ...
      .values({
        ...
        conceptsJson: JSON.stringify(content.concepts),
        updatedAt: now,
      })
      .onConflictDoUpdate({
        target: moduleGuides.moduleId,
        set: {
          summary: content.summary,
          conceptsJson: JSON.stringify(content.concepts),
          updatedAt: now,
...
export async function getGeneratedStudyContent(moduleId: number) {
  const guide = db
    .select()
    .from(moduleGuides)
    .where(eq(moduleGuides.moduleId, moduleId))
    .get();
  ...
  return {
    guide: guide
      ? {
          ...guide,
          concepts: JSON.parse(guide.conceptsJson) as unknown[],
        }
      : null,
    flashcards: db
      .select()
      .from(flashcards)
      .where(eq(flashcards.moduleId, moduleId))
      .all(),
    quizQuestions: db
      .select()
      .from(quizQuestions)
...
export async function getFlashcardForUser(
  ...
  return db
    ...
    .from(flashcards)
    .innerJoin(modules, eq(flashcards.moduleId, modules.id))
    .where(
      and(
...
export async function updateManualFlashcardForUser(
  flashcardId: number,
  userId: number,
  input: {
    prompt: string;
...
export async function exportModuleBackupForUser(
  ...
  return createModuleBackup({
    ...
    guide: content.guide
      ? {
          summary: content.guide.summary,
          concepts: content.guide.concepts,
        }
      : null,
    flashcards: content.flashcards.map((card) => ({
      prompt: card.prompt,
      answer: card.answer,
      explanation: card.explanation,
    })),
    quizQuestions: content.quizQuestions.map((question) => ({
      prompt: question.prompt,
      options: JSON.parse(question.optionsJson) as Array<{
        id: string;
        text: string;
      }>,
      correctOptionId: question.correctOptionId,
  ...
  db.transaction((tx) => {
    const result = tx
      ...
      .values({
        ...
        description: backup.module.description ?? null,
        createdAt: now,
        updatedAt: now,
      })
      .run();
  ...
  db.transaction((tx) => {
    ...
    tx
      ...
      .values({
        ...
        nextIntervalDays: input.intervalDays,
        reviewedAt: now,
      })
      .run();
  });
...
export async function addTutorMessage(input: {
  moduleId: number;
  userId: number;
  role: "user" | "assistant";
  content: string;
  ...
  const result = db
    ...
    .values({
      ...input,
      citedMaterialIdsJson: input.citedMaterialIds
        ? JSON.stringify(input.citedMaterialIds)
        : null,
      createdAt: new Date(),
    })
    .run();
...
export async function getAnalyticsData(userId: number) {
  ...
  const states = db
    .select()
    .from(cardReviewStates)
    .where(eq(cardReviewStates.userId, userId))
    .all();
...
export async function searchStudyContentForUser(
  ...
  const candidates = [
    ...
    ...notes.map((note) => ({
      ...
      moduleTitle:
        byId.get(note.moduleId)?.title ?? "Module",
      title: note.title,
      content: note.body,
    })),
    ...
    ...readyMaterials.map((material) => ({
      ...
      moduleTitle:
        byId.get(material.moduleId)?.title ?? "Module",
      title: material.originalFilename,
      content: material.extractedText ?? "",
    })),
  ];

  return searchCandidates(candidates, query);
}