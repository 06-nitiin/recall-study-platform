...
import { cardReviewStates, flashcards, materials, moduleGuides, moduleNotes, moduleTasks, modules, quizQuestions, quizResponses, reviewEvents, studyPreferences, studySessions, tutorMessages, users } from "../../drizzle/schema";
...
export async function updateUserDisplayName(userId: number, displayName: string) { db.update(users).set({ displayName, updatedAt: new Date() }).where(eq(users.id, userId)).run(); return findUserById(userId); }
export async function updateUserPassword(userId: number, passwordHash: string) { db.update(users).set({ passwordHash, updatedAt: new Date() }).where(eq(users.id, userId)).run(); return findUserById(userId); }
...
export async function listTasksForModuleUser(moduleId: number, userId: number) { return db.select({ task: moduleTasks }).from(moduleTasks).innerJoin(modules, eq(moduleTasks.moduleId, modules.id)).where(and(eq(moduleTasks.moduleId, moduleId), eq(modules.userId, userId))).orderBy(moduleTasks.completedAt, moduleTasks.dueDate, desc(moduleTasks.createdAt)).all().map((row) => row.task); }
export async function createTaskForModule(moduleId: number, input: { title: string; dueDate?: string | null }) { const now = new Date(); const result = db.insert(moduleTasks).values({ moduleId, title: input.title, dueDate: input.dueDate ?? null, createdAt: now, updatedAt: now }).run(); return db.select().from(moduleTasks).where(eq(moduleTasks.id, Number(result.lastInsertRowid))).get(); }
export async function getTaskForUser(taskId: number, userId: number) { return db.select({ task: moduleTasks }).from(moduleTasks).innerJoin(modules, eq(moduleTasks.moduleId, modules.id)).where(and(eq(moduleTasks.id, taskId), eq(modules.userId, userId))).get()?.task; }
export async function setTaskCompleteForUser(taskId: number, userId: number, completed: boolean) { const task = await getTaskForUser(taskId, userId); if (!task) return undefined; db.update(moduleTasks).set({ completedAt: completed ? new Date() : null, updatedAt: new Date() }).where(eq(moduleTasks.id, task.id)).run(); return db.select().from(moduleTasks).where(eq(moduleTasks.id, task.id)).get(); }
export async function deleteTaskForUser(taskId: number, userId: number) { const task = await getTaskForUser(taskId, userId); if (!task) return false; db.delete(moduleTasks).where(eq(moduleTasks.id, task.id)).run(); return true; }
...
export async function createManualFlashcard(moduleId: number, input: { prompt: string; answer: string; explanation?: string | null }) { const result = db.insert(flashcards).values({ moduleId, prompt: input.prompt, answer: input.answer, explanation: input.explanation ?? null, isGenerated: false, createdAt: new Date() }).run(); return db.select().from(flashcards).where(eq(flashcards.id, Number(result.lastInsertRowid))).get(); }
export async function getFlashcardForUser(flashcardId: number, userId: number) { return db.select({ card: flashcards }).from(flashcards).innerJoin(modules, eq(flashcards.moduleId, modules.id)).where(and(eq(flashcards.id, flashcardId), eq(modules.userId, userId))).get()?.card; }
export async function updateManualFlashcardForUser(flashcardId: number, userId: number, input: { prompt: string; answer: string; explanation?: string | null }) { const card = await getFlashcardForUser(flashcardId, userId); if (!card || card.isGenerated) return undefined; db.update(flashcards).set({ prompt: input.prompt, answer: input.answer, explanation: input.explanation ?? null }).where(eq(flashcards.id, card.id)).run(); return db.select().from(flashcards).where(eq(flashcards.id, card.id)).get(); }
export async function deleteManualFlashcardForUser(flashcardId: number, userId: number) { const card = await getFlashcardForUser(flashcardId, userId); if (!card || card.isGenerated) return false; db.delete(flashcards).where(eq(flashcards.id, card.id)).run(); return true; }
export async function createManualQuizQuestion(moduleId: number, input: { prompt: string; options: Array<{ id: string; text: string }>; correctOptionId: string; explanation?: string | null }) { const result = db.insert(quizQuestions).values({ moduleId, prompt: input.prompt, optionsJson: JSON.stringify(input.options), correctOptionId: input.correctOptionId, explanation: input.explanation ?? null, isGenerated: false, createdAt: new Date() }).run(); return db.select().from(quizQuestions).where(eq(quizQuestions.id, Number(result.lastInsertRowid))).get(); }
export async function getQuizQuestionForUser(questionId: number, userId: number) { return db.select({ question: quizQuestions }).from(quizQuestions).innerJoin(modules, eq(quizQuestions.moduleId, modules.id)).where(and(eq(quizQuestions.id, questionId), eq(modules.userId, userId))).get()?.question; }
export async function updateManualQuizQuestionForUser(questionId: number, userId: number, input: { prompt: string; options: Array<{ id: string; text: string }>; correctOptionId: string; explanation?: string | null }) { const question = await getQuizQuestionForUser(questionId, userId); if (!question || question.isGenerated) return undefined; db.update(quizQuestions).set({ prompt: input.prompt, optionsJson: JSON.stringify(input.options), correctOptionId: input.correctOptionId, explanation: input.explanation ?? null }).where(eq(quizQuestions.id, question.id)).run(); return db.select().from(quizQuestions).where(eq(quizQuestions.id, question.id)).get(); }
export async function deleteManualQuizQuestionForUser(questionId: number, userId: number) { const question = await getQuizQuestionForUser(questionId, userId); if (!question || question.isGenerated) return false; db.delete(quizQuestions).where(eq(quizQuestions.id, question.id)).run(); return true; }
...
export async function createFocusSession(userId: number, moduleId: number) { const result = db.insert(studySessions).values({ userId, moduleId, kind: "focus", startedAt: new Date(), durationSeconds: 0 }).run(); return db.select().from(studySessions).where(eq(studySessions.id, Number(result.lastInsertRowid))).get(); }
export async function finishFocusSession(userId: number, sessionId: number) { const session = db.select().from(studySessions).where(and(eq(studySessions.id, sessionId), eq(studySessions.userId, userId), eq(studySessions.kind, "focus"))).get(); if (!session || session.endedAt) return undefined; const { elapsedFocusSeconds } = await import("../lib/focusSessions"); const endedAt = new Date(); const durationSeconds = elapsedFocusSeconds(session.startedAt, endedAt); db.update(studySessions).set({ endedAt, durationSeconds }).where(eq(studySessions.id, session.id)).run(); return { durationSeconds, endedAt }; }
...
export async function getAnalyticsData(userId: number) { const ownedModules = await listModulesForUser(userId); const events = db.select().from(reviewEvents).where(eq(reviewEvents.userId, userId)).all(); const states = db.select().from(cardReviewStates).where(eq(cardReviewStates.userId, userId)).all(); const sessions = db.select().from(studySessions).where(eq(studySessions.userId, userId)).all(); const tasks = db.select({ task: moduleTasks }).from(moduleTasks).innerJoin(modules, eq(moduleTasks.moduleId, modules.id)).where(eq(modules.userId, userId)).all().map((row) => row.task); return { modules: ownedModules, events, states, sessions, tasks }; }

export async function searchStudyContentForUser(userId: number, query: string) { const ownedModules = await listModulesForUser(userId); const byId = new Map(ownedModules.map((module) => [module.id, module])); const notes = db.select({ note: moduleNotes }).from(moduleNotes).innerJoin(modules, eq(moduleNotes.moduleId, modules.id)).where(eq(modules.userId, userId)).all().map((row) => row.note); const tasks = db.select({ task: moduleTasks }).from(moduleTasks).innerJoin(modules, eq(moduleTasks.moduleId, modules.id)).where(eq(modules.userId, userId)).all().map((row) => row.task); const readyMaterials = db.select({ material: materials }).from(materials).innerJoin(modules, eq(materials.moduleId, modules.id)).where(and(eq(modules.userId, userId), eq(materials.extractionStatus, "ready"))).all().map((row) => row.material); const candidates = [...ownedModules.map((module) => ({ type: "module" as const, moduleId: module.id, moduleTitle: module.title, title: module.title, content: module.description ?? "" })), ...notes.map((note) => ({ type: "note" as const, moduleId: note.moduleId, moduleTitle: byId.get(note.moduleId)?.title ?? "Module", title: note.title, content: note.body })), ...tasks.map((task) => ({ type: "task" as const, moduleId: task.moduleId, moduleTitle: byId.get(task.moduleId)?.title ?? "Module", title: task.title, content: task.dueDate ? `Due ${task.dueDate}` : "" })), ...readyMaterials.map((material) => ({ type: "material" as const, moduleId: material.moduleId, moduleTitle: byId.get(material.moduleId)?.title ?? "Module", title: material.originalFilename, content: material.extractedText ?? "" }))]; return searchCandidates(candidates, query); }
